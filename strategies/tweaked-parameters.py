...
for index, row in data.iterrows():
    ...
    if shares_held > 0 and price <= entry_price * (1 - STOP_LOSS_PERCENT):
        ...
        commission = shares_held * TRANSACTION_COST  # Commission per share
        total_revenue = revenue - commission  # Deduct commission from revenue
        balance += total_revenue  # Add the net revenue to the balance
    ...
    # Buy Signal
        ...
        if shares_to_buy > 0:
            ...
            commission = shares_to_buy * TRANSACTION_COST  # Commission per share
            total_cost = cost + commission  # Include commission in the total cost
            balance -= total_cost  # Deduct the total cost from the balance
